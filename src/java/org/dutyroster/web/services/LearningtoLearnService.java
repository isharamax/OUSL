...
//            session.update(cat);
                ...
                if (newCat == null) {
                    ...
                    session.save(newCat);
                    lerStaff.setPoints(getUserPoints(lerStaff));
...
//                } else if (cat.getVacantSlots() < distribution.getNoOfMembers() && cat.getVacantSlots() != 0) {
    ...
    public short getUserPoints(LearningStaff lerSchedule) {
        short X = 0;
        //short Y = 0;
        short Z = 0;
        short points = 0;
        ...
        try {
            ...
            q.setDate("holiday", lerSchedule.getId().getLerId().getLerDate());
            ...
            q.setEntity("staffMember", lerSchedule.getId().getStaffId());
            q.setEntity("department", lerSchedule.getId().getStaffId().getDepartment());
            ...
            if (workstation != null) {
                DayTypeWeghtageId id = new DayTypeWeghtageId(lerSchedule.getId().getStaffId().getCenter(), workstation, dayType);
                ...
                if (dayTypeWeghtage != null) {
                    ...
                    //System.out.println(dayTypeWeghtage);
                } else {
                    X = 0;
                }
            } else {
            ...
            if (workstation != null) {
                WorkstationCenterId workstationCenterId = new WorkstationCenterId(lerSchedule.getId().getStaffId().getCenter(), workstation);
